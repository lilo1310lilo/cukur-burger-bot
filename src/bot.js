...
const { showStaff, handleCallback: staffCallback, staffScenes } = require('./handlers/admin/staff');
...
const { globalAuth, checkRole } = require('./middleware/auth');
...
// Global Auth: notanishlarni ignore qilish
bot.use(globalAuth);
...
bot.hears('👥 Foydalanuvchilar', checkRole(['owner', 'admin']), showStaff);
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    ...
    // Xodimlar (Staff) callback lari
    if (data.startsWith('staff_')) {
      return await staffCallback(ctx);
    }
