...
const addMenuItemScene = new Scenes.WizardScene(
  ...
  async (ctx) => {
    ...
    const buttons = cats.map(c => [Markup.button.callback(c.name_uz, `addcat_${c.id}`)]);
  ...
  async (ctx) => {
    ...
    const catId = parts[1];
    ctx.wizard.state.item.category_id = catId;
    const cats = await require('../../db').getCategories();
    const c = cats.find(c => c.id === catId);
    ctx.wizard.state.item.cat_name = c ? c.name_uz : 'Kategoriya';
