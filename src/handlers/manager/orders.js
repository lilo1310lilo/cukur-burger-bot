...
async function handleCallback(ctx) {
  ...
  if (data.startsWith('order_reject_') && !data.startsWith('order_reject_reason_')) {
    ...
    return ctx.editMessageText(
      ...
      { parse_mode: 'HTML', ...Markup.inlineKeyboard([
        [Markup.button.callback('💳 Chek soxta', `rej_1_${id}`)],
        [Markup.button.callback('🔍 Chek o\'qib bo\'lmaydi', `rej_2_${id}`)],
        [Markup.button.callback('💰 Summa mos emas', `rej_3_${id}`)],
        [Markup.button.callback('⏰ Kafe yopiq', `rej_4_${id}`)],
        [Markup.button.callback('❌ Boshqa sabab', `rej_5_${id}`)],
  ...
  if (data.startsWith('rej_')) {
    const reasonKey = data.split('_')[1]; // 1, 2, 3, 4, 5
    const id = data.replace(`rej_${reasonKey}_`, '');
    const mapReason = {
      '1': 'Chek soxta', '2': 'Chek o\'qib bo\'lmaydi',
      '3': 'Summa mos emas', '4': 'Kafe yopiq', '5': 'Boshqa sabab'
    };
    const reason = mapReason[reasonKey];
    if (!reason) return;
